...
from models.transformer.ops import LayerNorm
from models.prior.autoregressive import ConditionalAutoregressive2D
from models.prior.conditioners import Conditioner, LabelConditioner, Condition_Modes
from models.utils.torch_utils import assert_shape
from models.utils.dist_utils import print_once
from models.vqvae.vqvae import calculate_strides
...
class SimplePrior(nn.Module):
    def __init__(self, z_shapes, l_bins, encoder, decoder, level,
                 ...
                 merged_decoder=False, single_enc_dec=False, clip_emb=False, video_clip_emb=False):
        super().__init__()
        ...
        if single_enc_dec:
            ...
            if self.n_tokens != 0 and self.use_tokens:
                from models.transformer.ops import Conv1D
    ...
    def sample(self, n_samples, z=None, z_conds=None, y=None, fp16=False, temp=1.0, top_k=0, top_p=0.0,
               chunk_size=None, sample_tokens=None, cfg_s=0):
        N = n_samples
            ...
            if self.single_enc_dec:
                ...
                if no_past_context:
                    z = self.prior.sample(n_samples, x_cond, y_cond, encoder_kv, fp16=fp16, temp=temp, top_k=top_k,
                                          top_p=top_p, sample_tokens=sample_tokens, cfg_s=cfg_s)
                else:
                    z = self.prior.primed_sample(n_samples, z, x_cond, y_cond, encoder_kv, fp16=fp16, temp=temp,
                                             top_k=top_k, top_p=top_p, chunk_size=chunk_size, sample_tokens=sample_tokens, cfg_s=cfg_s)
