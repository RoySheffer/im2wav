...
from models.transformer.ops import filter_logits
from models.transformer.transformer import Transformer
from models.utils.logger import get_range
from models.utils.torch_utils import empty_cache
from models.utils.dist_utils import use_cuda
...
class ConditionalAutoregressive2D(nn.Module):
    ...
    def get_logits(self, x_cond, y_cond, sample_t, n_samples, x, fp16,encoder_kv, top_k, top_p, temp, i=0):
        ...
        if i > 0:
            ...
            x = self.transformers[1](x, encoder_kv=encoder_kv, sample=True, fp16=fp16)  # Transformer
        else:
    ...
    def sample(self, n_samples, x_cond=None, y_cond=None, encoder_kv=None, fp16=False, temp=1.0, top_k=0, top_p=0.0,
               get_preds=False, sample_tokens=None, cfg_s=0):
            ...
            for sample_t in get_range(range(0, sample_tokens)):
                if isinstance(y_cond, list):
                    ...
                    for i in range(len(y_cond)):
                        ...
                        # prob = t.distributions.Categorical(logits=logits)
                    ...
                    x = t.distributions.Categorical(logits=logits_cfg).sample()  # Sample and replace x
                    # x = t.distributions.Categorical(probs=(0.5 * prob_cur[0].probs + 0.5*prob_cur[1].probs)).sample()  # Sample and replace x
                else:
                    ...
                    x = t.distributions.Categorical(logits=logits).sample()  # Sample and replace x
    ...
    def primed_sample(self, n_samples, x, x_cond=None, y_cond=None, encoder_kv=None, fp16=False, temp=1.0, top_k=0,
                      top_p=0.0, get_preds=False, chunk_size=None, sample_tokens=None, cfg_s=0):
        assert self.training == False
        ...
        if self.x_cond:
            ...
            if isinstance(x_cond, list):
                for x in x_cond:
                    assert x.shape == (N, D, self.width) or x_cond.shape == (
        ...
        with t.no_grad():
            ...
            x_primes = []
            print(f"\nchunk_sizes={chunk_sizes} len(xs)={len(xs)}, chunk_size={chunk_size}\n")
