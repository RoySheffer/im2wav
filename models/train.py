...
print(f"adding {modules_dir} modules path")
import fire
import warnings
import numpy as np
import torch as t
...
from models.make_models import make_vqvae, make_prior, restore_opt, save_checkpoint
...
from models.utils.dist_utils import print_once, allreduce, allgather
...
def log_inputs(orig_model, logger, x_in, y, x_out, hps, tag="train"):
    ...
    bs = x_in.shape[0]
    if hps.prior:
        if hps.labels and (not hps.clip_emb):
...
def sample_prior(orig_model, ema, logger, x_in, y, hps):
    ...
    if hps.prior and hps.labels and (not hps.clip_emb):
...
def save_random_sample(model, samples_num, dir, sr=16000):
    import soundfile
    import os
    if not os.path.exists(dir):
        os.makedirs(dir)
    with t.no_grad():
        x_sample = model.sample(samples_num)
        aud = t.clamp(x_sample, -1, 1).cpu().numpy()
        for i in list(range(aud.shape[0])):
            soundfile.write(f'{dir}/item_{i}.wav', aud[i], samplerate=sr, format='wav')
...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    for i, x in logger.get_range(data_processor.train_loader):
        ...
        #save_checkpoint(logger, 'latest', orig_model, opt, dict(step=logger.iters), hps)
        # Forward
        x_out, loss, _metrics = model(x, **forw_kwargs)
        ...
        if logger.iters % 200000 == 0:
            print("torch.cuda.memory_allocated: %fGB" % (t.cuda.memory_allocated(0) / 1024 / 1024 / 1024))
            print("torch.cuda.memory_reserved: %fGB" % (t.cuda.memory_reserved(0) / 1024 / 1024 / 1024))
            print("torch.cuda.max_memory_reserved: %fGB" % (t.cuda.max_memory_reserved(0) / 1024 / 1024 / 1024))
        ...
        # Sample
        with t.no_grad():
            # if logger.iters % 300000 == 1:
            #     print(f"iter {logger.iters} saving random sample")
            #     save_random_sample(model=model, samples_num=2, dir=f"{logger.logdir}/samples/{logger.iters}", sr=16000)
