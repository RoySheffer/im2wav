...
def setup_dist_from_mpi(master_addr="127.0.0.1", backend="gloo", port=29500, n_attempts=5, verbose=False):
    if not isWindows and backend=="gloo":
        pass
        # backend = "nccl"
...
def _setup_dist_from_mpi(master_addr, backend, port, n_attempts, verbose):
    ...
    for attempt_idx in range(n_attempts):
        os.environ["MASTER_PORT"] = str(port + attempt_idx)
        try:
