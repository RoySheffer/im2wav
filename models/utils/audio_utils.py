...
from models.utils.dist_utils import isWindows
...
def calculate_bandwidth(dataset, hps, duration=600):
    ...
    if dataset.cumsum[-1]< n_samples:
        print(f"data set total duration is {dataset.cumsum[-1]} samples and you are creating a data set of size {n_samples} samples")
    ...
    if dist.is_available() and (not isWindows) and False:
        from models.utils.dist_utils import allreduce
