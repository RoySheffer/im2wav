...
def filter_logits(logits, top_k=0, top_p=0.0, filter_value=-float('Inf')):
    ...
    # cumulative_probs_shift = t.zeros_like(cumulative_probs)
    # cumulative_probs_shift[:, 0, 1:] = cumulative_probs[:, 0, :-1]
    # p = t.distributions.Categorical(logits=sorted_logits).probs
    # tresh = 0.005
    # k_print = 5
    # num_relevant = (p[:,0] > tresh).sum(dim=1)
    # print(f"\nhighest {k_print} prob bins \n {p[:, 0, :k_print]} \n bins above {tresh}:\n {num_relevant} \n cum probability to get all bins above {tresh} should be higher than:\n {[cumulative_probs_shift[i, 0, num_relevant[i]-1] for i in range(logits.shape[0])]}", "\n-------------------")
